  ...
  return (
    <PageShell size="wide" className="space-y-8 sm:space-y-12">
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        {actions.map((action) => (
          <Link key={action.href} href={action.href} className="premium-card group rounded-3xl border border-white/10 bg-white/[0.035] p-5 transition hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.06]">
            ...
            <span className="mt-5 inline-flex text-sm font-semibold text-cyan-100 transition group-hover:text-white">View →</span>
          </Link>
        ))}
      </section>
      ...
      <SectionCard className="space-y-5">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-white sm:text-3xl">Top Rankings</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">Open a category to load only the top 10 using live Yahoo Finance quote data.</p>
          </div>
          <PremiumButton href="/rankings" className="w-full sm:w-auto">All rankings</PremiumButton>
        </div>
        <div className="grid gap-3 md:grid-cols-3">
          {rankingPreview.map((item) => (
            <Link key={item.href} href={item.href} className="rounded-2xl border border-white/10 bg-black/20 p-4 transition hover:border-cyan-200/30 hover:bg-white/[0.045]">
              <p className="text-xs uppercase tracking-[0.24em] text-slate-500">{item.label}</p>
              <h3 className="mt-3 text-lg font-semibold text-white">{item.title}</h3>
              <p className="mt-4 text-sm font-medium text-cyan-100">Load top 10 →</p>
            </Link>
          ))}
        </div>
      </SectionCard>
