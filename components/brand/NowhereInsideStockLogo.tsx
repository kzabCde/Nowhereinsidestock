...
export default function NowhereInsideStockLogo({ compact = false, href = "/", className = "" }: NowhereInsideStockLogoProps) {
  ...
  return (
    <Link
      ...
      className={`group relative inline-flex max-w-full items-center overflow-hidden px-1 py-0.5 ${className}`}
    >
      <span
        className={`relative z-10 block truncate font-extrabold uppercase leading-tight tracking-[0.08em] text-transparent ${textSize}
          [-webkit-text-stroke:1.25px_rgba(226,232,240,0.9)] [text-stroke:1.25px_rgba(226,232,240,0.9)]
          transition-[color,text-shadow,-webkit-text-stroke-color] duration-300 ease-out
      ...
      <span
        aria-hidden="true"
        className="pointer-events-none absolute inset-x-1 bottom-0 h-[2px] origin-left scale-x-0 bg-gradient-to-r from-white/75 via-cyan-300/75 to-violet-400/80 shadow-[0_0_12px_rgba(139,92,246,0.45)] transition-transform duration-300 group-hover:scale-x-100 group-active:scale-x-100"
      />
    </Link>
  );
}
