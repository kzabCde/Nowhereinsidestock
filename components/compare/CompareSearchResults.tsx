...
export function CompareSearchResults({ results, loading, query, onAdd }: { results: SearchSymbolItem[]; loading: boolean; query: string; onAdd: (symbol: string) => void }) {
  ...
  if (query.trim() && results.length === 0) return <p className="mt-3 text-sm text-slate-300">{locale === "th" ? "ไม่พบผลลัพธ์" : "No results found."}</p>;
  return (
    <div className="mt-3 grid gap-2 sm:grid-cols-2">
      {results.map((item) => (
        <article key={item.symbol} draggable onDragStart={(e) => e.dataTransfer.setData("text/plain", item.symbol)} className="rounded-2xl border border-white/10 bg-white/[0.025] p-3">
          ...
          <p className="truncate text-xs text-slate-400">{item.name ?? item.shortname ?? (locale === "th" ? "ไม่ทราบชื่อ" : "Unknown")}</p>
          <button type="button" onClick={() => onAdd(item.symbol)} className="btn-premium mt-3 min-h-8 rounded-xl px-3 py-1 text-xs">{locale === "th" ? "เพิ่มเพื่อเปรียบเทียบ" : "Add to Compare"}</button>
        </article>
      ))}
    </div>
  );
